...
/*DNB metastore*/
resource "aws_s3_bucket" "dnb_metastore" {
...
# DNB metastore public access block for bucket
resource "aws_s3_bucket_public_access_block" "dnb_metastore_pub_block" {
  bucket = aws_s3_bucket.dnb_metastore.id
...
# DNB metastore Role in AWS
#trust relationships policy json
data "aws_iam_policy_document" "dnb_metastore_assume_role_policy" {
  statement {
    effect  = "Allow"
...
resource "aws_iam_role" "dnb_metastore_role" {
  name               = "${var.prefix}-dnb-metastore"
  assume_role_policy = data.aws_iam_policy_document.dnb_metastore_assume_role_policy.json
  tags               = var.tags
}

#inline policy json text
data "aws_iam_policy_document" "dnb_metastore_policy" {
  statement {
    effect = "Allow"
    ...
    "s3:PutObjectAcl"]
    resources = [
      "${aws_s3_bucket.dnb_metastore.arn}/*",
    aws_s3_bucket.dnb_metastore.arn]
  }
}

#inline policy apply
resource "aws_iam_role_policy" "dnb_metastore_role_policy" {
  name   = "${var.prefix}-dnb-metastore-policy"
  role   = aws_iam_role.dnb_metastore_role.id
  policy = data.aws_iam_policy_document.dnb_metastore_policy.json
}
